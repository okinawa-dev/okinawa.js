...
Engine.GUI.GuiBubble.prototype = Object.create(Engine.GUI.GuiElement.prototype);
...
Engine.GUI.GuiBubble.prototype.initialize = function(text)
{
  ...
  this._text.setSize(this.size.x - this._borderWidth * 2 - 10, this.size.y - this._borderWidth * 2 - 10);
...
Engine.GUI.GuiBubble.prototype.draw = function(ctx)
{
  ...
  if (this._arrowPosition < 3)
    ctx.lineTo(arrowPos.x, arrowPos.y + 15);
  else
    ctx.lineTo(arrowPos.x, arrowPos.y - 16);
...
Engine.GUI.GuiBubble.prototype.setBackgroundColor = function(color) { this._backgroundColor = color; };
Engine.GUI.GuiBubble.prototype.setBorderColor = function(color) { this._borderColor = color; };
Engine.GUI.GuiBubble.prototype.setBorderWidth = function(width) { this._borderWidth = width; };
Engine.GUI.GuiBubble.prototype.setCornerRadius = function(radius) { this._cornerRadius = radius; };

Engine.GUI.GuiBubble.prototype.setArrowPosition = function(pos)
{
  if ((pos < 0) || (pos > Object.keys(Engine.GUI.GuiBubble.ARROW_POSITIONS).length))
    return;

  this._arrowPosition = pos;
};

Engine.GUI.GuiBubble.prototype.getText = function() { return this._text; };

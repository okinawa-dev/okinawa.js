...
Font.prototype.validate = function(target, zero, mark, font, timeout) {
  if (timeout !== false && timeout < 0) {
    this.onerror(
      "Requested system font '" +
        this.fontFamily +
        "' could not be loaded (it may not be installed)."
    );
    return;
  }
  var width = getComputedStyle(target, null)
  ...
  // font has not finished loading - wait 50ms and try again
    ...
    setTimeout(function() {
      font.validate(
        ...
        mark,
        font,
        timeout === false ? false : timeout - 50
      );
    }, 1000);
  }
};
...
Font.prototype.ondownloaded = function() {
  ...
  // terminal error: stop running code
    ...
    error(
      'Error: file at ' + this.url + ' cannot be interpreted as OpenType font.'
    );
    return;
  }
  ...
  var checkTableError = function(tag) {
    if (!tags[tag]) {
      error(
        "Error: font is missing the required OpenType '" + tag + "' table."
      );
    ...
    if (version === 4) {
      ...
      if (endChar != false) {
        ...
        var newcode =
          ...
          chr16(endChar) +
          chr16(0xffff) + // "endCount" array
          chr16(0) + // cmap required padding
          chr16(endChar) +
          chr16(0xffff) + // "startCount" array
...
Font.prototype.bootstrapValidation = function(printChar, timeout) {
  ...
  if (typeof getComputedStyle === 'undefined') {
    this.onload();
    error(
      'Error: getComputedStyle is not supported by this browser.\n' +
...
Font.prototype.measureText = function(textString, fontSize) {
  ...
  metrics.ascent = 0;
  metrics.descent = 0;
  metrics.bounds = { minx: 0, maxx: metrics.width, miny: 0, maxy: 0 };
  metrics.height = 0;
...
Font.prototype.measureSegment = function(textSegment, fontSize, metrics) {
  ...
  var scanwidth = (metrics.width + padding) | 0,
    ...
    pixelData = ctx.getImageData(x_offset, y_offset, scanwidth, scanheight)
      .data;
